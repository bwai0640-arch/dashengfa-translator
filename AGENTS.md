...
- 取词顺序是 Word / PowerPoint COM、Windows UI Automation、安全剪贴板兼容读取。不得覆盖图片、文件或并发更新的剪贴板，不得读取密码框。
...
- 修改代码后至少运行 `python -m py_compile desktop_app.py selection_capture.py app.py` 和 `python -m unittest discover -s tests -v`；修改打包后还要启动成品 EXE 并检查本地模型、托盘与日志。
